async function generateImages(
  ...
  sessionStorage.setItem("imageGroupID", data.data.access_token); //store the token in session storage
...
document
  ...
  .addEventListener("click", async function (e) {
    ...
    let popup = document.getElementById("mobius-loader");
    ...
    generateImages(4, prompt, categories, promptImageId, templateImageId)
      .then((response) => {
        ...
        if (response.data && response.data.image_group_id) {
          sessionStorage.setItem("imageGroupId", response.data.image_group_id);
        }
      })
      .catch((error) => {
        console.error(error);
